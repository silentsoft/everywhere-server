...
public class TbmSysUserDQM extends AbstractDAO {
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public List getUserBySeqAndPassword(final Map inputMap) {
		...
		sql.append("  FROM TBM_SYS_USER \n");
		sql.append(" WHERE 1=1 \n");
		sql.append("   AND USER_SEQ = :userSeq \n");
		...
		Map<String, Object> data = new HashMap<String, Object>();
		if (countMap(inputMap) > 0) {
			if (isValidKey(inputMap, "userSeq")) {
				data.put("userSeq", inputMap.get("userSeq"));
			}
			if (isValidKey(inputMap, "userPwd")) {
				data.put("userPwd", inputMap.get("userPwd"));
			}
		}
